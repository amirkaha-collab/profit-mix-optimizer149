...
# ═══════════════════════════════════════════════════════════════════
# LANDING PAGE  (additive only — zero changes to existing logic)
# ═══════════════════════════════════════════════════════════════════
...
/* ── Landing hero ── */
.lp-hero {
  background: linear-gradient(135deg, #0f2240 0%, #1F3A5F 55%, #1a4fa0 100%);
  border-radius: 16px; padding: 48px 52px 44px; margin-bottom: 32px;
  position: relative; overflow: hidden;
}
.lp-hero::after {
  content: ''; position: absolute; inset: 0; border-radius: 16px;
  background: radial-gradient(ellipse at 80% 20%, rgba(58,122,254,.25) 0%, transparent 60%);
  pointer-events: none;
}
.lp-hero-eyebrow {
...
.lp-hero-sub {
  font-size: 15px; color: #93c5fd; line-height: 1.6;
  max-width: 560px; margin: 0 0 30px;
}
.lp-hero-pills { display: flex; gap: 8px; flex-wrap: wrap; }
.lp-hero-pill {
  background: rgba(255,255,255,.1); border: 1px solid rgba(255,255,255,.2);
  border-radius: 999px; padding: 4px 13px; font-size: 11px;
...
.lp-section-hdr {
  ...
  text-transform: uppercase; color: #3A7AFE; margin-bottom: 4px;
}
.lp-section-title {
...
.lp-card::before {
  ...
  background: var(--card-accent, #EFF6FF); opacity: .7;
}
.lp-card-icon { font-size: 28px; margin-bottom: 12px; display: block; line-height: 1; }
.lp-card-title { font-size: 16px; font-weight: 800; color: #111827; margin-bottom: 6px; }
.lp-card-desc { font-size: 12.5px; color: #6B7280; line-height: 1.6; margin-bottom: 16px; min-height: 52px; }
.lp-card-tags { display: flex; gap: 5px; flex-wrap: wrap; margin-bottom: 14px; }
.lp-card-tag {
  font-size: 10px; font-weight: 700; padding: 2px 8px;
  border-radius: 999px; background: #F0F4FF; color: #1F3A5F;
}

/* ── Steps timeline ── */
.lp-steps { display: flex; gap: 0; }
.lp-step {
...
.lp-step:not(:last-child)::after {
  ...
  background: linear-gradient(90deg, #3A7AFE, #93c5fd);
  z-index: 0;
}
.lp-step-num {
  width: 40px; height: 40px; border-radius: 50%;
  background: linear-gradient(135deg, #1F3A5F, #3A7AFE);
  color: #fff; font-size: 14px; font-weight: 800;
  display: flex; align-items: center; justify-content: center;
  position: relative; z-index: 1; margin-bottom: 10px; flex-shrink: 0;
}
.lp-step-title { font-size: 12px; font-weight: 700; color: #111827; margin-bottom: 4px; }
.lp-step-desc  { font-size: 11px; color: #6B7280; line-height: 1.5; }

/* ── Flow cards ── */
.lp-flow-card {
  background: #F8FAFF; border: 1px solid #E0E9FF; border-radius: 12px;
  padding: 16px 18px; margin-bottom: 0;
}
.lp-flow-icon  { font-size: 18px; margin-bottom: 6px; display: block; }
.lp-flow-title { font-size: 13px; font-weight: 800; color: #1F3A5F; margin-bottom: 4px; }
.lp-flow-steps { font-size: 11.5px; color: #374151; line-height: 1.8; }
...
.lp-check-icon { font-size: 18px; flex-shrink: 0; margin-top: 1px; }
.lp-check-text { font-size: 12px; color: #374151; line-height: 1.5; }
.lp-check-text b { color: #111827; font-weight: 700; display: block; margin-bottom: 2px; }

/* ── Shortcut chips ── */
.lp-shortcuts { display: flex; gap: 8px; flex-wrap: wrap; }
.lp-shortcut {
  background: #fff; border: 1.5px solid #D1D5DB; border-radius: 999px;
  padding: 6px 16px; font-size: 12px; font-weight: 600; color: #374151;
...
.lp-disclaimer {
  ...
  padding: 12px 16px; font-size: 11px; color: #78350F; line-height: 1.6;
}
</style>
"""
...
def _nav_to(page: str, product_type: str = None):
    """Route to a page. Sets app_page + optionally product_type."""
    st.session_state["app_page"] = page
    if product_type:
        st.session_state["product_type"] = product_type
        # Clear results on product-type switch
        for k in ["last_results", "selected_alt", "run_history", "quick_profile_active"]:
            st.session_state[k] = [] if k == "run_history" else None
...
def render_landing_page(df_long: "pd.DataFrame"):
    """
    Full-screen landing page rendered when app_page == 'home'.
    Calls st.stop() at the end — nothing below executes.
    Zero changes to any existing logic or session state keys.
    """
    st.markdown(_LANDING_CSS, unsafe_allow_html=True)

    # ── HERO ─────────────────────────────────────────────────────────
    st.markdown("""
<div class="lp-hero" dir="rtl">
  <div class="lp-hero-eyebrow">Profit Mix Optimizer · v3.8</div>
  <div class="lp-hero-title">ניהול תיק פיננסי חכם<br>ממוקד ומדויק</div>
  <div class="lp-hero-sub">
    פלטפורמה מקצועית לאופטימיזציה של תמהיל מסלולי השקעה,
    ניתוח תיק לקוח מקיף, ועיון בנתוני אסטרטגיות מוסדיות.
  </div>
  <div class="lp-hero-pills">
    <span class="lp-hero-pill">📊 קרנות השתלמות</span>
    <span class="lp-hero-pill">📋 פוליסות חיסכון</span>
    <span class="lp-hero-pill">🏛️ קרנות פנסיה</span>
    <span class="lp-hero-pill">💼 קופות גמל</span>
    <span class="lp-hero-pill">📈 גמל להשקעה</span>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    st.markdown('<div class="lp-section-hdr">מה תרצה לעשות?</div>', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-title">בחר את מסלול העבודה שלך</div>', unsafe_allow_html=True)

    c1, c2, c3, c4 = st.columns(4, gap="medium")

    with c1:
        st.markdown("""
<div class="lp-card" style="--card-accent:#EFF6FF">
  <span class="lp-card-icon">⚙️</span>
  <div class="lp-card-title">אופטימיזציית תמהיל</div>
  <div class="lp-card-desc">מצא את שילוב המסלולים האופטימלי לפי יעדי חשיפה, מגבלות ומנהלים.</div>
  <div class="lp-card-tags">
    <span class="lp-card-tag">מניות</span>
    <span class="lp-card-tag">חו"ל</span>
    <span class="lp-card-tag">מט"ח</span>
    <span class="lp-card-tag">שארפ</span>
  </div>
</div>""", unsafe_allow_html=True)
        if st.button("➜ פתח אופטימיזציה", key="lp_btn_optimizer", use_container_width=True, type="primary"):
            _nav_to("app", "קרנות השתלמות")
            st.rerun()

    with c2:
        st.markdown("""
<div class="lp-card" style="--card-accent:#F0FDF4">
  <span class="lp-card-icon">📊</span>
  <div class="lp-card-title">ניתוח תיק לקוח</div>
  <div class="lp-card-desc">ייבא דוח מסלקה, קבל ניתוח מקיף של תיק הלקוח, תכנן שינויים והפק דוחות.</div>
  <div class="lp-card-tags">
    <span class="lp-card-tag">מסלקה</span>
    <span class="lp-card-tag">תכנון</span>
    <span class="lp-card-tag">השוואה</span>
    <span class="lp-card-tag">דוח</span>
  </div>
</div>""", unsafe_allow_html=True)
    ...
    with c3:
        st.markdown("""
<div class="lp-card" style="--card-accent:#FFF7ED">
  <span class="lp-card-icon">📐</span>
  <div class="lp-card-title">אסטרטגיות מוסדיות</div>
  <div class="lp-card-desc">עיין בנתוני האלוקציה ההיסטוריים של גופים מוסדיים לאורך זמן.</div>
  <div class="lp-card-tags">
    <span class="lp-card-tag">היסטוריה</span>
    <span class="lp-card-tag">גופים</span>
    <span class="lp-card-tag">טרנדים</span>
  </div>
</div>""", unsafe_allow_html=True)
        if st.button("➜ פתח ניתוח מוסדי", key="lp_btn_isa", use_container_width=True, type="primary"):
            _nav_to("app", "קרנות השתלמות")
            st.session_state["_lp_scroll_isa"] = True
            st.rerun()

    with c4:
        st.markdown("""
<div class="lp-card" style="--card-accent:#FDF4FF">
  <span class="lp-card-icon">📄</span>
  <div class="lp-card-title">דוחות ויצוא</div>
  <div class="lp-card-desc">הפק דוחות HTML, Jupyter Notebooks, NotebookLM packages וקובצי CSV.</div>
  <div class="lp-card-tags">
    <span class="lp-card-tag">HTML</span>
    <span class="lp-card-tag">Excel</span>
    <span class="lp-card-tag">Jupyter</span>
  </div>
</div>""", unsafe_allow_html=True)
        if st.button("➜ פתח דוחות", key="lp_btn_reports", use_container_width=True, type="primary"):
            _nav_to("app", "תיק לקוח")
            st.rerun()
    ...
    # ── HOW IT WORKS ─────────────────────────────────────────────────
    st.markdown('<div dir="rtl">', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-hdr">תהליך העבודה</div>', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-title">איך עובדים עם המערכת</div>', unsafe_allow_html=True)
    st.markdown("""
<div class="lp-steps">
  <div class="lp-step">
    <div class="lp-step-num">1</div>
    <div class="lp-step-title">בחירת מוצר</div>
    <div class="lp-step-desc">בחר קרן השתלמות, פוליסה, פנסיה, גמל או ניתוח תיק</div>
  </div>
  <div class="lp-step">
    <div class="lp-step-num">2</div>
    <div class="lp-step-title">הגדרת יעדים</div>
    <div class="lp-step-desc">הגדר יעדי חשיפה: מניות, חו"ל, מט"ח, לא-סחיר</div>
  </div>
  <div class="lp-step">
    <div class="lp-step-num">3</div>
    <div class="lp-step-title">חישוב חלופות</div>
    <div class="lp-step-desc">המערכת מחשבת אלפי שילובים ומחזירה את המיטביים</div>
  </div>
  <div class="lp-step">
    <div class="lp-step-num">4</div>
    <div class="lp-step-title">ניתוח השוואתי</div>
    <div class="lp-step-desc">השווה חלופות, ראה השפעה על תיק קיים, עיין בנתונים</div>
  </div>
  <div class="lp-step">
    <div class="lp-step-num">5</div>
    <div class="lp-step-title">הפקת דוח</div>
    <div class="lp-step-desc">ייצא דוח מקצועי ללקוח בפורמט HTML, Excel, או Notebook</div>
  </div>
</div>
""", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    # ── COMMON FLOWS ─────────────────────────────────────────────────
    st.markdown('<div dir="rtl">', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-hdr">תרחישי שימוש</div>', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-title">זרימות עבודה נפוצות</div>', unsafe_allow_html=True)

    fa, fb, fc = st.columns(3, gap="medium")
    with fa:
        st.markdown("""
<div class="lp-flow-card" dir="rtl">
  <span class="lp-flow-icon">🔄</span>
  <div class="lp-flow-title">ניוד קרן השתלמות</div>
  <div class="lp-flow-steps">
    1. ייבא דוח מסלקה<br>
    2. בחר טאב קרנות השתלמות<br>
    3. הגדר יעד תמהיל חדש<br>
    4. השווה חלופות<br>
    5. הפק דוח ייעוץ
  </div>
</div>""", unsafe_allow_html=True)
...
<div class="lp-flow-card" dir="rtl">
  <span class="lp-flow-icon">📋</span>
  <div class="lp-flow-title">ניתוח תיק חדש</div>
  <div class="lp-flow-steps">
    1. פתח ניתוח תיק לקוח<br>
    2. העלה דוח מסלקה<br>
    3. צפה בניתוח גרפי מלא<br>
    4. תכנן שינויים עתידיים<br>
    5. שלח דוח ל-NotebookLM
  </div>
</div>""", unsafe_allow_html=True)
...
<div class="lp-flow-card" dir="rtl">
  <span class="lp-flow-icon">📐</span>
  <div class="lp-flow-title">בחינת אסטרטגיה מוסדית</div>
  <div class="lp-flow-steps">
    1. פתח אופטימיזציה<br>
    2. גלול לניתוח אסטרטגיות<br>
    3. בחר גוף ומסלול<br>
    4. עיין בטרנד היסטורי<br>
    5. השווה לתמהיל יעד
  </div>
</div>""", unsafe_allow_html=True)
    ...
    st.markdown('<div dir="rtl">', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-hdr">הכנה מוקדמת</div>', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-title">מה כדאי להכין לפני שמתחילים</div>', unsafe_allow_html=True)
    st.markdown("""
<div class="lp-check-grid">
  <div class="lp-check-item">
    <span class="lp-check-icon">📥</span>
    <div class="lp-check-text">
      <b>דוח מסלקה</b>
      קובץ XLSX מהמסלקה הפנסיונית — נדרש לניתוח תיק קיים ולהשוואת לפני/אחרי
    </div>
  </div>
  <div class="lp-check-item">
    <span class="lp-check-icon">🎯</span>
    <div class="lp-check-text">
      <b>יעדי חשיפה</b>
      הגדרת אחוזי מניות, חו"ל, מט"ח, לא-סחיר — המספרים שמגדירים את פרופיל הסיכון
    </div>
  </div>
  <div class="lp-check-item">
    <span class="lp-check-icon">🏢</span>
    <div class="lp-check-text">
      <b>העדפת מנהלים</b>
      האם יש גופים מנהלים מועדפים? האם מותר לערבב? מה הכמות המקסימלית?
    </div>
  </div>
  <div class="lp-check-item">
    <span class="lp-check-icon">💰</span>
    <div class="lp-check-text">
      <b>היקפי השקעה</b>
      סכום כולל לחלוקה — נדרש לחישוב משקל קרן נעולה ולדוח ערכי
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    # ── SHORTCUTS ────────────────────────────────────────────────────
    st.markdown('<div dir="rtl">', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-hdr">קיצורי דרך</div>', unsafe_allow_html=True)
    st.markdown('<div class="lp-section-title">כניסה מהירה</div>', unsafe_allow_html=True)

    sc1, sc2, sc3, sc4, sc5 = st.columns(5, gap="small")
    _shortcuts = [
        (sc1, "🏦 קרנות השתלמות", "app", "קרנות השתלמות"),
        (sc2, "📋 פוליסות חיסכון", "app", "פוליסות חיסכון"),
        (sc3, "🏛️ קרנות פנסיה",   "app", "קרנות פנסיה"),
        (sc4, "💼 קופות גמל",      "app", "קופות גמל"),
        (sc5, "📈 גמל להשקעה",    "app", "גמל להשקעה"),
    ]
    for col, label, page, pt in _shortcuts:
        with col:
            if st.button(label, key=f"lp_sc_{pt}", use_container_width=True):
                _nav_to(page, pt)
                st.rerun()
    ...
    # ── DISCLAIMER ───────────────────────────────────────────────────
    st.markdown("""
<div class="lp-disclaimer" dir="rtl">
  ⚠️ <b>הצהרה:</b> המערכת מיועדת לסיוע בקבלת החלטות ניהול השקעות בלבד ואינה מהווה ייעוץ פיננסי מחייב.
  הנתונים המוצגים מבוססים על מידע ציבורי וכפופים לעדכונות ושינויים.
  יש לאמת נתונים מהמקורות הרשמיים לפני קבלת כל החלטת השקעה.
</div>
""", unsafe_allow_html=True)
...
# ── ROUTING: show landing page if app_page == "home" ──
